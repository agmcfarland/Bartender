...
# run_dir <- data.frame(run_dir = '/data/bar_lab/small_scale_barcode_analyses/data/interim/1-bnab_at_arti/bartender_run_stock_and_experimental_group1_and_group2')
# 
...
df_plot_text <- df_plot %>%
  ...
  # 
  dplyr::mutate(
    tile_label = ifelse(in_stock == TRUE, paste(100 * round(trusted_proportion, 2), '\n', format_thousands(unique, 0), '|', format_thousands(total, 0)), paste(format_thousands(unique, 0), '|', format_thousands(total, 0)))
    ) %>%
