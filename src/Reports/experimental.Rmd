...
# params <- data.frame(run_dir = '/data/bar_lab/small_scale_barcode_analyses/data/interim/1-beat_villinger_peterson/bartender_output')
# params <- data.frame(run_dir = '/data/bar_lab/small_scale_barcode_analyses/data/interim/1-bnab_2_0/bartender_output')
# params$experimental_trend_width <- 40
# params$experimental_trend_height <- 20
# params$per_experimental_group_width <- 15
# params$per_experimental_group_height <- 15
...
```{r, experimental_trend, message=FALSE, warning=FALSE, echo=FALSE, out.width = '100%',fig.align="center", fig.width = dynamic_figure_manager$experimental_trend_width, fig.height=dynamic_figure_manager$experimental_trend_height}
rm(list = list_non_global_variables(ls(), keep_global_vars))
...
This section shows proportion of barcodes, the number of unique barcodes, and the total abundance of barcodes for each biological group according to a more refined view of the stock sourcing. Here, barcodes are color coded by what specific stock they were found in. If a barcode is not foud in any stock, they are labeled as "experimental". If they are found in two or more different stocks then they are labeled "stock overlap". Otherwise, they are labeled with the name of stock.
