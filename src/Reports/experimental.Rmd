...
# run_dir <- data.frame(run_dir = '/data/bar_lab/small_scale_barcode_analyses/data/interim/1-SHIV_D_CNS/bartender_run_stock_and_experimental')
# 
...
```{r, per_experimental_group_specific_stock_source, message=FALSE, warning=FALSE, echo=FALSE, out.width = '100%',fig.align="center", fig.width=15, fig.height=15}
rm(list = list_non_global_variables(ls(), keep_global_vars))
...
if (length(base::unique(read.csv(file.path(runs_paths$stock, 'stock_barcodes_filtered.csv'))$biological_group)) > 1) {
  ...
  for (biological_group_ in base::unique(df_record$biological_group)) {break}
