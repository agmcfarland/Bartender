...
# def hamming_distance(string1, string2, cutoff = 1):
# 	'''
# 	Computes the hamming distance between two strings up until the string_distance is greater than cutoff+1 and returns that value.
# 	'''
...
def hamming_distance_preset_length(string1, string2, barcode_length = 35):
	'''
...
def hamming_worker(barcode1, barcodes2_list, barcode_length, cutoff):
	...
	for barcode2 in barcodes2_list:
		distance = hamming_distance_preset_length(string1 = barcode1, string2 = barcode2, barcode_length = barcode_length, cutoff = cutoff)
		if distance <= cutoff:
...
def hamming_distance_array(barcodes1_list, barcodes2_list, barcode_length, cutoff):
	...
	with mp.Pool() as pool:
		results = [pool.apply_async(hamming_worker, (barcode1, barcodes2_list, barcode_length, cutoff)) for barcode1 in barcodes1_list]
