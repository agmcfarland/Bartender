...
class Experimental:
    ...
    def _samples_to_process(self):
        self.incoming_samples = {
            i: {"path": [], "barcode": pd.DataFrame()}
            for i in self.setup_manager.experimental["standard_sample_name"].tolist()
        }

        for _, row in self.setup_manager.experimental.iterrows():
            self.incoming_samples[row["standard_sample_name"]]["path"].append(
                pjoin(
                    self.setup_manager.run_paths.temp,
                    row["sample_name"],
                    row["standard_sample_name"] + "_barcode.csv",
                )
            )
    ...
    def _get_counts_of_incoming_samples(self):
        for standard_sample_name, data in self.incoming_samples.items():
            for filename in data["path"]:
                standard_sample_name_alphanumeric_id = (
                    standard_sample_name + "_" + self._create_alphanumeric_id()
                )

                df = pd.read_csv(filename)

                df.rename(
                    {standard_sample_name: standard_sample_name_alphanumeric_id},
                    inplace=True,
                    axis=1,
                )
                ...
                if data["barcode"].shape == (0, 0):
                    ...
                    data["barcode"] = data["barcode"].merge(
                        df, on="barcode", how="outer"
                    )
    ...
    def _merge_incoming_samples_with_existing_samples(self):
        for standard_sample_name, data in self.incoming_samples.items():
            if os.path.exists(
                pjoin(
                    self.setup_manager.run_paths.experimental,
                    standard_sample_name + "_barcode.csv",
                )
            ):
                df = pd.read_csv(
                    pjoin(
    ...
    def _get_mean_template_value(self, standard_sample_name):
        return self.setup_manager.record.experimental[
            self.setup_manager.record.experimental["standard_sample_name"]
            == standard_sample_name
        ]["input_template"].mean()

    def _get_proportion_filter(self, standard_sample_name):
        return float(
            1 / self._get_mean_template_value(standard_sample_name=standard_sample_name)
        )
    ...
    def _get_minimum_passing_stock_proportion(self, df, standard_sample_name):
        ...
        if len(df) != 0:
            ...
            return self._get_proportion_filter(
                standard_sample_name=standard_sample_name
            )
    ...
    def _get_best_hamming_match(self, df_experimental, barcodes1_list, barcodes2_list):
        ...
        barcode_distances = hamming_distance_array(
            barcodes1_list=barcodes1_list,
            barcodes2_list=barcodes2_list,
        ...
        barcode_distances = pd.DataFrame(
            barcode_distances, columns=["query", "reference", "distance"]
        )
        barcode_distances = barcode_distances.merge(
            df_experimental[["barcode", "total"]],
            left_on="reference",
            right_on="barcode",
            how="inner",
        )
        ...
        barcode_distances = barcode_distances[
            (barcode_distances["distance"] > 0)
            & (barcode_distances["distance"] <= self.cutoff)
        ]

        # get the index of the largest reference_count for each query and subset
        best_query_hit_id = barcode_distances.groupby("query")[
            "reference_count"
        ].idxmax()
    ...
    def _get_max_stock_barcode(self, stock_barcodes):
        ...
        """
        stock_barcodes = (
            stock_barcodes.groupby("barcode")["total_stock"].max().reset_index()
        )
    ...
    def _apply_barcode_filters(self, standard_sample_name, df, stock_barcodes):
        ...
        barcode_distances = self._get_best_hamming_match(
            ...
            barcodes2_list=set(
                df["barcode"].tolist() + stock_barcodes["barcode"].tolist()
            ),
        )
        barcode_distances.rename(
            {
                "reference": "best_match_sequence",
                "distance": "best_match_distance",
                "reference_count": "best_match_count",
            },
            inplace=True,
            axis=1,
        )
        ...
        df = df.merge(
            barcode_distances, left_on="barcode", right_on="query", how="left"
        )
        df["pcr_error_check"] = df.apply(
            lambda x: "pass" if x["best_match_count"] * 0.05 < x["total"] else "fail",
            axis=1,
        )
        df["pcr_error_check"] = df.apply(
            lambda x: "not_applicable"
            if pd.isna(x["best_match_sequence"])
            else x["pcr_error_check"],
            axis=1,
        )
        ...
        df["instock_threshold_check"] = df.apply(
            lambda x: "pass"
            if (x["total_stock"] > 0)
            & (
                x["proportion"]
                > self._get_proportion_filter(standard_sample_name=standard_sample_name)
            )
            else "fail",
            axis=1,
        )
        df["instock_threshold_check"] = df.apply(
            lambda x: "not_applicable"
            if pd.isna(x["total_stock"])
            else x["instock_threshold_check"],
            axis=1,
        )
        ...
        df["unique_barcode_threshold_check"] = df.apply(
            lambda x: "pass"
            if (pd.isna(x["total_stock"]))
            & (
                x["proportion"]
                > self._get_minimum_passing_stock_proportion(
                    df=df, standard_sample_name=standard_sample_name
                )
            )
            else "fail",
            axis=1,
        ...
        df["unique_barcode_threshold_check"] = df.apply(
            lambda x: "not_applicable"
            if x["total_stock"] > 0
            else x["unique_barcode_threshold_check"],
            axis=1,
        )

        # flag trusted barcodes
        df["trusted_barcode_check"] = df.apply(
            lambda x: "pass"
            if (
                x["pcr_error_check"] == "pass"
                or x["pcr_error_check"] == "not_applicable"
            )
            & (
                x["instock_threshold_check"] == "pass"
                or x["instock_threshold_check"] == "not_applicable"
            )
            & (
                x["unique_barcode_threshold_check"] == "pass"
                or x["unique_barcode_threshold_check"] == "not_applicable"
            )
            else "fail",
            axis=1,
        )
        ...
        df["trusted_proportion"] = df.apply(
            lambda x: x["total"] / sum_of_trusted_barcodes
            if x["trusted_barcode_check"] == "pass"
            else float(0),
            axis=1,
        )
    ...
    def make_filtered_barcodes(self):
        ...
        stock_barcodes = self._adjust_filtered_stock_format(
            stock_barcodes=self.stock._read_filtered_stock_barcodes()
        )

        for standard_sample_name in (
            self.setup_manager.record.experimental["standard_sample_name"]
            .unique()
            .tolist()
        ):
            ...
            if len(df) > 0:
                df.to_csv(
                    pjoin(
                        self.setup_manager.run_paths.experimental,
                        standard_sample_name + "_processed.csv",
                    ),
                    index=None,
                )
