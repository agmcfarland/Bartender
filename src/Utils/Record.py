...
class Record:
	...
	def _check_unique_values_in_column(self, table, column_name):
		return len(table[column_name].unique().tolist()) == len(table[column_name].tolist())
		
	def _check_all_R1_unique(self, table):
		if self._check_unique_values_in_column(table = table, column_name = 'R1') is not True:
			raise ValueError('R1 must must be unique')

	def _check_all_R2_unique(self, table):
		if self._check_unique_values_in_column(table = table, column_name = 'R2') is not True:
			raise ValueError('R2 must must be unique')

	def _check_unique_names(self, table):
		table['unique_name'] = table.apply(lambda x: x['sample_name'] + '_' + x['standard_sample_name'], axis = 1)

		if self._check_unique_values_in_column(table = table, column_name = 'unique_name') is not True:
	...
	def write_record(self, record_type):
		...
		record_paths = {'experimental': self.run_paths.record_experimental, 'stock': self.run_paths.record_stock}
