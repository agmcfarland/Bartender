...
class SetupManager:
    ...
    def _verify_bartender_table_inputs(
        self, df, allowed_columns: list, table_type: str
    ):
        ...
        if len(df.columns) != len(allowed_columns):
            raise ValueError(
                f"{table_type}: Input length ({len(df.columns)}) does not equal expected length ({len(allowed_columns)})"
            )
    ...
    def _read_input_experimental(self, experimental_file):
        return self._verify_bartender_table_inputs(
            df=pd.read_csv(pjoin(experimental_file)),
            allowed_columns=self._allowed_experimental_columns(),
            table_type="experimental",
        )

    def _read_input_stock(self, stock_file):
        return self._verify_bartender_table_inputs(
            df=pd.read_csv(pjoin(stock_file)),
            allowed_columns=self._allowed_stock_columns(),
            table_type="stock",
        )

    def _assign_standardized_sample_names_stock(self):
        self.stock["standard_sample_name"] = self.stock.apply(
            lambda x: x["biological_group"] + "_" + x["sample_name"], axis=1
        )

    def _assign_standardized_sample_names_experimental(self):
        self.experimental["standard_sample_name"] = self.experimental.apply(
