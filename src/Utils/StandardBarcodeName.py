...
class StandardBarcodeName:
    ...
    def _read_in_stock_barcode(self):
        return pd.read_csv(
            pjoin(self.setup_manager.run_paths.stock, "stock_barcodes_filtered.csv")
        )
    ...
    def _assign_stock_barcode_names(self):
        ...
        df["barcode_id_base"] = df.apply(
            lambda x: x["biological_group"]
            if x["observations"] == 1
            else "stock_" + str(x["observations"]),
            axis=1,
        )
        ...
        df["standard_barcode_name"] = (
            df["barcode_id_base"] + "_" + df["barcode_order"].astype(str)
        )
    ...
    def assign_standard_barcode_names(self):
        ...
        if len(df_experimental) != 0:
            df_experimental = df_experimental.merge(
                df_stock, on="barcode", how="outer", indicator=True
            )

            df_experimental = df_experimental[df_experimental["_merge"] == "left_only"]

            df_experimental = self._assign_unique_experimental_barcode_names(
                df=df_experimental
            )
    ...
    def _assign_unique_experimental_barcode_names(self, df):
        ...
        df["standard_barcode_name"] = (
            "experimental_"
            + df["observations"].astype(str)
            + "_"
            + df["barcode_order"].astype(str)
        )
