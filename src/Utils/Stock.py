...
class Stock:
    ...
    def _update_raw_stock_table(self):
        ...
        for sample_file in glob(
            pjoin(self.setup_manager.run_paths.temp, "*", "*_barcode.csv")
        ):
            df_stock = df_stock.merge(
                pd.read_csv(sample_file), on="barcode", how="outer"
            )
    ...
    def _get_mean_template_value(self, biological_group):
        return self.setup_manager.record.stock[
            self.setup_manager.record.stock["biological_group"] == biological_group
        ]["input_template"].mean()

    def _get_proportion_filter(self, biological_group):
        return float(
            1 / self._get_mean_template_value(biological_group=biological_group)
        )
    ...
    def _filter_by_proportion(self, df, biological_group):
        return df[
            df["proportion_barcode"]
            >= self._get_proportion_filter(biological_group=biological_group)
        ]
    ...
    def make_filtered_stock_table(self):
        ...
        for _biological_group in self._get_unique_biological_groups(df_stock):
            ...
            df_biological_group = self._select_biological_group(
                df=df_biological_group, biological_group=_biological_group
            )

            df_biological_group[
                "stocks_with_barcode"
            ] = self._count_stocks_with_barcode(df=df_biological_group)

            df_biological_group = self._filter_on_singletons(
                df=df_biological_group, count_threshold=1
            )

            df_biological_group["total_barcodes"] = self._get_total_barcode_reads(
                df=df_biological_group
            )

            df_biological_group["proportion_barcode"] = self._get_barcode_proportion(
                df=df_biological_group
            )

            df_biological_group = self._filter_by_proportion(
                df=df_biological_group, biological_group=_biological_group
            )
