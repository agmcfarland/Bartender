...
def make_run1_inputs(test_path):
    ...
    df_experimental.to_csv(
        pjoin(test_path, "experimental_samplesheet_run1.csv"), index=None
    )
...
def make_run2_inputs(test_path):
    ...
    df_experimental.to_csv(
        pjoin(test_path, "experimental_samplesheet_run2.csv"), index=None
    )
...
def make_run3_inputs(test_path):
    ...
    df_experimental.to_csv(
        pjoin(test_path, "experimental_samplesheet_run3.csv"), index=None
    )
...
def test_run1(
    test_path, test_workdir, make_workdir, make_run1_inputs, make_run2_inputs
):
    """
    ...
    assert pd.read_csv(
        pjoin(
            ...
            "work",
            "experimental",
            "animal1_unknown_27_plasma_dna_processed.csv",
        )
    ).shape == (1188, 12)
    ...
    assert pd.read_csv(
        pjoin(
            ...
            "work",
            "experimental",
            "animal2_unknown_27_plasma_dna_processed.csv",
        )
    ).shape == (399, 12)
    ...
    assert pd.read_csv(
        pjoin(
            ...
            "work",
            "experimental",
            "animal1_unknown_27_plasma_dna_barcode.csv",
        )
    ).shape == (2439, 3)
    ...
    assert pd.read_csv(
        pjoin(
            ...
            "work",
            "experimental",
            "animal2_unknown_27_plasma_dna_barcode.csv",
        )
    ).shape == (930, 2)


def test_run1_and_run2(
    test_path, test_workdir, make_workdir, make_run1_inputs, make_run2_inputs
):
    """
...
def test_run1_and_run2_run3(
    ...
    assert os.path.exists(
        pjoin(
            ...
            "work",
            "experimental",
            "animal3_unknown_27_plasma_dna_processed.csv",
        )
    )
...
def test_run1_and_run2_run3_run4(
    test_path,
    test_workdir,
    make_workdir,
    make_run1_inputs,
    ...
    assert pd.read_csv(
        pjoin(test_path, "work", "stock", "stock_barcodes_raw.csv")
    ).shape == (10430, 6)

    assert pd.read_csv(
        pjoin(test_path, "work", "stock", "stock_barcodes_filtered.csv")
    ).shape == (5485, 5)
