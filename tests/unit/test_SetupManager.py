...
def mock_os_makedirs():
    with patch("Utils.FilePaths.os.makedirs") as mocked_makedirs:
        mocked_makedirs.side_effect = [
            "/path/to/workdir",
            "/path/to/workdir/record",
            "/path/to/workdir/stock",
            "/path/to/workdir/experimental",
            "/path/to/workdir/output",
        ]
...
def test_incorrect_columns(
    mock_os_makedirs, correct_experimental_input, correct_stock_input
):
    """
    ...
    with patch("pandas.read_csv") as mock_read_csv:
        with patch("os.path.exists") as mock_path_exists:
            ...
            df_test_experimental.rename(
                {"biological_group": "animal_id"}, inplace=True, axis=1
            )
            ...
            with pytest.raises(
                ValueError, match="experimental: animal_id not in columns"
            ):
                bartender_setup = SetupManager(
                    work_dir="path/to/workdir",
                    experimental_file="path/to/experimental.csv",
    ...
    with patch("pandas.read_csv") as mock_read_csv:
        with patch("os.path.exists") as mock_path_exists:
            ...
            with pytest.raises(
                ValueError,
                match=re.escape(
                    "experimental: Input length (9) does not equal expected length (10)"
                ),
            ):
                bartender_setup = SetupManager(
                    work_dir="path/to/workdir",
...
def test_R1_path_does_not_exist(
    mock_os_makedirs, correct_experimental_input, correct_stock_input
):
    with patch("pandas.read_csv") as mock_read_csv:
        with patch("os.path.exists") as mock_path_exists:
            ...
            with pytest.raises(
                ValueError,
                match="experimental: path/to/sample1_R1.fastq.gz does not exist",
            ):
                bartender_setup = SetupManager(
                    work_dir="path/to/workdir",
                    experimental_file="path/to/experimental.csv",
