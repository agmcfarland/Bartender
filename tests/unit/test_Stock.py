...
def test_update_raw_stock_table(raw_barcode_tables, mock_setup_manager, test_data_dir):
    ...
    with patch("os.path.exists") as mock_path_exists:
        with patch("pandas.read_csv") as mock_read_csv:
            with patch("Bartender.Utils.Stock.glob") as mock_glob:
    ...
    df_reference_raw_stock = pd.read_csv(
        pjoin(test_data_dir, "stock", "stock_barcodes_raw.csv")
    )

    df_barcode_order = pd.DataFrame(
        {
            "barcode": df_reference_raw_stock["barcode"].tolist(),
            "order": [
                e for e, i in enumerate(df_reference_raw_stock["barcode"].tolist())
            ],
        }
    )

    df_reference_raw_stock = df_reference_raw_stock.merge(
        df_barcode_order, on="barcode"
    )
...
def test_make_filtered_stock_table(
    raw_barcode_tables, mock_setup_manager, test_data_dir
):
    """
    ...
    with patch("pandas.read_csv") as mock_read_csv:
        ...
        with patch.object(
            stock, "_get_unique_biological_groups", return_value=["CD4_derived_TF"]
        ):
            with patch.object(stock, "_get_mean_template_value", return_value=50000):
                df_filtered_stock = stock.make_filtered_stock_table()

    df_reference_filtered = pd.read_csv(
        pjoin(test_data_dir, "stock", "stock_barcodes_filtered.csv")
    )

    for col in list(df_filtered_stock.columns):
        if col == "proportion_barcode":
            proportion_experimental = [
                round(i, 2) for i in df_filtered_stock[col].tolist()
            ]
            proportion_reference = [
                round(i, 2) for i in df_reference_filtered[col].tolist()
            ]

            assert proportion_experimental == proportion_reference
        else:
            assert (
                df_filtered_stock[col].tolist() == df_reference_filtered[col].tolist()
            )
